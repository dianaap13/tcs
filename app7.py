...
<div class="header">
    ...
    <div class="header-subtitle">Análisis inteligente de comentarios</div>
</div>
    ...
    if df.empty:
        ...
        try:
            ...
            with PdfPages(pdf_path) as pdf:
                ...
                # Gráfico de categorías si existen los datos
            ...
            # Mover esta parte fuera del bloque with PdfPages
            try:
                with open(pdf_path, "rb") as f:
                    st.download_button(
                        "Descargar PDF", 
                        f, 
                        file_name="reporte_feedback.pdf", 
                        mime="application/pdf",
                        use_container_width=True
                    )
            except Exception as e:
                st.error(f"Error al preparar el PDF para descarga: {str(e)}")
            ...
            ax.text(0.5, 0.95, titulo, fontsize=24, fontweight='bold', ha='center', color=colores_empresa[0])
        ...
        with open(pdf_path, "rb") as f:
            st.download_button("📥 Descargar PDF", f, file_name="reporte_general_quejas.pdf", mime="application/pdf", use_container_width=True)
...
for i, (nombre, valor) in enumerate(metricas.items()):
    with [col1, col2, col3, col4][i]:
        ...
        <div class="metric-card">
            ...
            <div class="metric-value">{valor:.1f if isinstance(valor, (int, float)) else valor}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if not df.empty and 'predicted_category' in df.columns:
        ...
        custom_cmap = LinearSegmentedColormap.from_list("empresa", colores_empresa)
        ...
        if 'tokens' in df.columns:
            try:
                ...
                if all_tokens:
                    ...
                    # Crear colormap personalizado
                    custom_colormap = LinearSegmentedColormap.from_list("empresa", colores_empresa)
